...
def load_dns_data(config: TrainingConfig) -> Tuple[torch.Tensor, ...]:
    ...
    RSTE = np.genfromtxt("Re2000_bal_k.dat",comments="%")
...
def train_pinn(
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        if (epoch - start_epoch) % 100 == 0:
            ...
            print('[' + '#' * (epoch // (total_epochs // 50)) + '#' + '-' * (50 - epoch // (total_epochs // 50)) + ']' +'     '+ f' {np.round((epoch/total_epochs)*100, 2) + 0.02}% completed')                
            print("-" * 80, '\n')
...
#Checkpoint to resume training to Aron/pinn_model.pth
CHECKPOINT_IN = './pinn_model0.pth'
CHECKPOINT_OUT = None  # Optional checkpoint after first epoch
#----------------------------------------------
