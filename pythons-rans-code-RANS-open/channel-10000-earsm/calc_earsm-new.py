...
def calc_earsm(k2d,om2d,u2d_face_w,u2d_face_s,v2d_face_w,v2d_face_s,uu2d,uv2d,vv2d,vis2d_earsm):
    ...
    global  dudy2_scaled_min,dudy2_scaled_max,N2_min,N2_max,scaler_dudy2,scaler_N2,neural_net,uu_loc,vv_loc,uv_loc,rk_loc,\
     N2_loc, uv_tot
...
# load pytorch model
      ...
      print('dudy2_scaled_min,dudy2_scaled_max,N2_min,N2_max ',dudy2_scaled_min,dudy2_scaled_max,N2_min,N2_max)
      print('b1_min,b1_max, b2_min,b2_max, b4_min,b4_max', b1_min,b1_max, b2_min,b2_max, b4_min,b4_max)
...
#   np.savetxt('y_u_k_ed_dudy.txt', np.c_[yp2d[0,:],u2d[0,:], k2d[0,:], diss[0,:], dudy[0,:]])
    ...
    vist = np.maximum(vis2d_earsm - viscos,1.e-10)
...
# dudy_squared_scaled[30],N2_DNS 13.860660216943005 0.8237877727000448
#   dudy_squared_scaled[0] = 13.860660216943005
#   N2[0] = 0.8237877727000448
...
#   print('beta1[0],beta2,beta4',beta1[30],beta2[30],beta4[30])

    print('np.min(beta1),max',np.min(beta1),np.max(beta1))
    print('np.min(beta2),max',np.min(beta2),np.max(beta2))
    print('np.min(beta4),max',np.min(beta4),np.max(beta4))
...
#   vv=2/3*rk  \
    ...
    print('min(uu)',np.min(uu))
    print('min(vv)',np.min(vv))
    uu = np.maximum(1.e-10,uu)
    vv = np.maximum(1.e-10,vv)
...
#   uv_tot = uv2d - (vis2d_earsm-viscos)*(dudy+dvdx)

#   print('uu[0,0],vv,uv,k',\
#  uu_loc[0,30]+0.6666*rk_loc[0,30],vv_loc[0,30]+0.6666*rk_loc[0,30],uv_tot[0,30],rk[0,30])
#   sys.exit()
