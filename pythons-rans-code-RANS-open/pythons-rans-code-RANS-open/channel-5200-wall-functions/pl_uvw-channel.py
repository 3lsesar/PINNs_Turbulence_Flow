...
tau_hist=np.loadtxt('ustar-history.dat')
ustar_hist_s=tau_hist[:,0]
ustar_hist_n=tau_hist[:,1]

ustar_s=np.mean(ustar_hist_s[-100:-1])
ustar_n=np.mean(ustar_hist_n[-100:-1])
...
fig1,ax1 = plt.subplots()
plt.subplots_adjust(left=0.20,bottom=0.20)
plt.plot(ustar_hist_s,'b-')
plt.plot(ustar_hist_n,'r-')
plt.ylabel(r"$u_\tau$")
plt.savefig('ustar-history.png',bbox_inches='tight')
