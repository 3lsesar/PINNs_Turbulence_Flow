...
# use absolute convergence criterium
   ...
   if solver_local == 'cgs':
      ...
      phi,info=linalg.cgs(A,su,x0=phi, tol=tol_conv, atol=abs_tol,  maxiter=nmax)  # good
   if solver_local == 'cg':
      if iter == 0:
         print('solver in solve_2d: cg')
      phi,info=linalg.cg(A,su,x0=phi, tol=tol_conv, atol=abs_tol,  maxiter=nmax)  # good
   if solver_local == 'gmres':
      if iter == 0:
         print('solver in solve_2d: gmres')
      phi,info=linalg.gmres(A,su,x0=phi, tol=tol_conv, atol=abs_tol,  maxiter=nmax)  # good
   if solver_local == 'qmr':
      if iter == 0:
         print('solver in solve_2d: qmr')
      phi,info=linalg.qmr(A,su,x0=phi, tol=tol_conv, atol=abs_tol,  maxiter=nmax)  # good
   if solver_local == 'lgmres':
      if iter == 0:
         print('solver in solve_2d: lgmres')
      phi,info=linalg.lgmres(A,su,x0=phi, tol=tol_conv, atol=abs_tol,  maxiter=nmax)  # good
   if info > 0:
