...
# boundary conditions for omega
   ...
   om_bc_east=np.ones(nj)
   om_bc_south=np.ones(ni)
   om_bc_north=np.ones(ni)
   ...
   om_bc_north=6*viscos/0.075/dist2_s
   om_bc_west=6*viscos/0.075/dist2_s
   om_bc_east=6*viscos/0.075/dist2_s
...
def modify_init(u2d,v2d,k2d,om2d,vis2d):
   ...
   k2d=np.ones((ni,nj))*0.01
   om2d=np.ones((ni,nj))*10
...
def calc_earsm(k2d,om2d,u2d_face_w,u2d_face_s,v2d_face_w,v2d_face_s,uu2d,uv2d,vv2d,ww2d,vis2d_earsm):
    ...
    global neural_net,pk_min, pk_max,scaler_yplus, scaler_pk,yplus_min, yplus_max, beta1_min, beta4_min, beta1_max, beta4_max, dudy,ttau,pk,vis2d_earsm1, beta2_min, beta2_max

    if iter == 0:
      print('NN calc_earsm called')
      folder='../NN/'
...
# load data
      ...
      scaler_pk = load(str(folder)+'model-scaler-pk.bin')
      scaler_yplus = load(str(folder)+'model-scaler-yplus.bin')

      beta1_min, beta1_max, beta2_min, beta2_max, beta4_min, beta4_max, pk_min, pk_max, yplus_min, yplus_max = np.loadtxt(str(folder)+'min-max.txt')

#     beta1_max = -0.08
#     beta2_min = 0.03
#     beta4_max = -0.04

      print('pk_min, pk_max',pk_min, pk_max, yplus_min, yplus_max)
      print('beta1_min, beta1_max',beta1_min, beta1_max)
      print('beta2_min, beta2_max',beta2_min, beta2_max)
      print('beta4_min, beta4_max',beta4_min, beta4_max)
    ...
    s21=s12
    s22=ttau*dvdy
    II_S=(s11**2+s12**2+s21**2+s22**2)  # s_ij s_ji = s_ij s_ij
...
#   if iter  == 0:
#   if iter % 100  == 0:
    pk = vist*dudy**2*viscos
#   pk = -uv_tot*dudy*viscos

    yplus = dist/viscos
      
# limit min/max
# count values larger/smaller than max/min
    pk_min_number= (pk < pk_min).sum()
    pk_max_number= (pk > pk_max).sum()

    print('pk_min_number',pk_min_number)
    print('pk_max_number',pk_max_number)
...
# set limits
    pk=np.minimum(pk,pk_max)
    pk=np.maximum(pk,pk_min)
...
#transform from tensor to numpy
    ...
    beta1 = np.reshape(beta1,(ni,nj))
    beta2 = np.reshape(beta2,(ni,nj))
    beta4 = np.reshape(beta4,(ni,nj))
      
# limit min/max
# count values larger/smaller than max/min
    beta1_min_number= (beta1< beta1_min).sum()
    beta1_max_number= (beta1> beta1_max).sum()

    print('beta1_min_number',beta1_min_number)
    print('beta1_max_number',beta1_max_number)

    beta4_min_number= (beta4< beta4_min).sum()
    beta4_max_number= (beta4> beta4_max).sum()

    print('beta4_min_number',beta4_min_number)
    print('beta4_max_number',beta4_max_number)

# set limits
    beta1=np.minimum(beta1,beta1_max)
    beta1=np.maximum(beta1,beta1_min)

    beta2=np.minimum(beta2,beta2_max)
    beta2=np.maximum(beta2,beta2_min)

    beta4=np.minimum(beta4,beta4_max)
    beta4=np.maximum(beta4,beta4_min)

#   uu=rk*beta4*(s12*om21-om12*s21)   # +rk*beta1*s11;  this is included via vis_earsm
#   vv=rk*beta4*(s21*om12-om21*s12)   # +rk*beta1*s22;   this is included via vis_earsm
    uu=rk*(beta4*(s12*om21-om12*s21)+beta2*(s11**2+s12**2-II_S/3))   # = a_11; 2/3*dk/dx_1 is omitted. It only affect the pressure
    vv=rk*(beta4*(s21*om12-om21*s12)+beta2*(s22**2+s12**2-II_S/3))   # = a_22; 2/3*dk/dx_2 is omitted. It only affect the pressure
    ww=-rk*beta2*II_S/3  
    uu = np.maximum(uu+0.6666*k2d,0)-0.6666*k2d
    vv = np.maximum(vv+0.6666*k2d,0)-0.6666*k2d
#   ww = np.maximum(ww+0.6666*k2d,0)-0.6666*k2d
...
# c_k = 1t except when PINN is used, see folder 
...
def read_restart_data():
   ...
   return u2d,v2d,p2d,k2d,om2d,vis2d,ap2d_vel

def save_data(u2d,v2d,p2d,k2d,om2d,vis2d,ap2d_vel):
...
# initialize
if pinn: 
   prand_k, c_k, c_omega_2 = modify_PINN()

# initialize
u2d,v2d,k2d,om2d,vis2d,dist=modify_init(u2d,v2d,k2d,om2d,vis2d)

# read data from restart
if restart: 
   u2d,v2d,p2d,k2d,om2d,vis2d,ap2d_vel= read_restart_data()
...
# boundary conditions for k2d
         ...
         aw3d,ae3d,as3d,an3d,ap3d,su3d,sp3d=fix_k()
...
# scale residuals
      ...
      resmax=np.max([residual_u ,residual_v,residual_pp])
